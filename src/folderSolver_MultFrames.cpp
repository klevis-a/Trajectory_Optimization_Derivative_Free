...
int main(int argc, char** argv)
{
    ...
    for(auto i=directory_iterator(p); i!=directory_iterator(); i++)
    {
        ...
        {
            auto jointsFile = regex_replace(fileName, regex("smoothFrames"), "jointsTF" + to_string(rowNum));
            auto cartesianFile = regex_replace(fileName, regex("smoothFrames"), "cartesianTF" + to_string(rowNum));
